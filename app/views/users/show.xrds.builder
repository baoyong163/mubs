...
xml.xrds(:XRDS,
  ...
  'xmlns' => 'xri://$xrd*($v*2.0)') do
    ...
    xml.Service(:priority => 1) do
      ...
      # xml.Type OpenID::PAPE::AUTH_MULTI_FACTOR if APP_CONFIG['use_ssl'] && @user.has_otp_device?
      # xml.Type OpenID::PAPE::AUTH_PHISHING_RESISTANT if APP_CONFIG['use_ssl'] && @user.has_otp_device?
  		xml.URI endpoint_url
  		xml.LocalID identity_url(:user => @user, :protocol => scheme)
    end
    xml.Service(:priority => 2) do
      ...
      # xml.Type OpenID::PAPE::AUTH_PHISHING_RESISTANT if APP_CONFIG['use_ssl'] && @user.has_otp_device?
  		xml.URI endpoint_url
  		xml.tag!('openid:Delegate', identity_url(:user => @user, :protocol => scheme))
    end
